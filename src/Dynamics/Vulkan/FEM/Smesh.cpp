...
void Smesh_t::loadFile(string filename)
{
	...
	int dummy;
	filein >> dummy >> dummy;
    m_points.resize(num_points, dyno::Vec3f(0.0f));
...
void Smesh_t::loadNodeFile(std::string filename)
{
	fstream filein(filename);
	if (!filein.is_open())
	{
...
void Smesh_t::loadNodeFile(std::string filename, Vec3f move)
{
	fstream filein(filename);
	if (!filein.is_open())
	{
...
void Smesh_t::loadTriangleFile(std::string filename)
{
	fstream filein(filename);
	if (!filein.is_open())
	{
...
void Smesh_t::loadTetFile(std::string filename)
{
	fstream filein(filename);
	if (!filein.is_open())
	{
